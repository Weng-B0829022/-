...
# ════════════════════════════════════════════════════════
# 5. R-B 三分法機制（Layer 3b 升級版）
#
#  問題根源：原 R-B 只做「GLOBAL / 繼續」二分法，
#            沒有路徑產生 LOCAL，導致全局只有 GLOBAL。
#
#  修正設計：用 B 值做三分法
#    B > T_high = Mean(B) + alpha × Std(B)  → GLOBAL（跨群組的真正全局橋梁）
#    T_low < B ≤ T_high                     → LOCAL（當前 k 層局部橋梁）
#    B ≤ T_low = Median(B)                  → 繼續往下層
#
#  alpha 語意：
#    alpha 越大 → T_high 越高 → GLOBAL 越嚴格，LOCAL 範圍越寬
#    alpha 越小 → T_high 接近 Mean → 更多邊判為 GLOBAL
#    alpha = 0  → T_high = Mean(B)，最嚴格的 GLOBAL 判定（約一半以上判為 GLOBAL）
#    alpha = 1  → T_high = Mean + Std，預設，GLOBAL/LOCAL 分界清晰
#
#  為何用 Mean+Std 而非 Median+MAD 做 T_high：
#    樹狀網路中 MAD 常為 0（多數邊 B 值相同），退化失效；
#    Mean+Std 對 B 值的右尾分布更敏感，能區分真正的高 B 值邊。
# ════════════════════════════════════════════════════════
...
def compute_betweenness_thresholds(G, candidate_edges, alpha=1.0):
    """
    計算 R-B 三分法的兩個閾值。

    T_high = Mean(B) + alpha × Std(B)   → 判 GLOBAL 的下界
    T_low  = Median(B)                  → 判 LOCAL 的下界（同時是「繼續下層」的上界）

    當 T_high ≤ T_low 時（極端情況）自動修正為 T_high = T_low + 1，
    保留三分法語意。

    回傳：(b_values_array, T_high, T_low, stats_dict)
    """
    ...
    T_high = mean_b + alpha * std_b
    T_low  = median_b

    # 防止三分法退化（T_high ≤ T_low 時強制拉開）
    if T_high <= T_low:
        T_high = T_low + 1.0

    stats = {
        'mean_B': mean_b, 'std_B': std_b,
        'median_B': median_b, 'mad_B': mad_b,
        'T_high': T_high, 'T_low': T_low,
    }
    return b_vals, T_high, T_low, stats
...
def heta_rb(G, n_random=100, alpha=1.0):
    """
    HETA + Layer 3b R-B 三分法機制。

    參數
    ----
    alpha : float ≥ 0
        控制 GLOBAL 判定嚴格度：
            T_high = Mean(B) + alpha × Std(B)
        alpha=0 → T_high = Mean(B)，最嚴格，約半數以上判 GLOBAL
        alpha=1 → T_high = Mean + Std，預設，GLOBAL/LOCAL 分界清晰
        alpha=2 → T_high = Mean + 2×Std，更寬鬆，LOCAL 範圍更大
    ...
    """
    if alpha < 0:
        raise ValueError(f"alpha 必須 ≥ 0，收到 alpha={alpha}")
    ...
    # ── Step 3.2: 逐層識別 Bond / Local（含 R-B 三分法）─
    for k in range(1, kmax + 1):
        ...
        # ── Layer 3b 介入點：T_I 退化時啟動三分法 ────────
        if is_ti_degenerate(candidate_ratios):
            b_vals, T_high, T_low, stats = compute_betweenness_thresholds(
                G, candidate_edges, alpha=alpha
            )

            n_global = n_local = n_pass = 0
            ...
            print(f"  [R-B 三分法] layer={k}  候選邊={len(candidate_edges)}  "
                  f"T_I={T_I:.4f}(退化)  alpha={alpha}  "
                  f"T_high={stats['T_high']:.1f}  T_low={stats['T_low']:.1f}  "
                  f"→ GLOBAL={n_global}  LOCAL={n_local}  pass={n_pass}")
...
if __name__ == '__main__':
    ...
    # ── alpha 掃描：R-B 三分法的 alpha 語意已改變
    #    alpha=0 → T_high=Mean(B)，最嚴格 GLOBAL 判定
    #    alpha=1 → T_high=Mean+Std，預設，GLOBAL/LOCAL 分界清晰
    #    alpha=2 → T_high=Mean+2×Std，更寬鬆，LOCAL 佔比更高
    # ────────────────────────────────────────────────────
        ...
        if log:
            ...
            for entry in log:
                print(f"    layer {entry['layer']}：候選邊 {entry['n_candidates']} | "
                      f"Mean(B)={entry['mean_B']:.1f}  Std(B)={entry['std_B']:.1f}  "
                      f"T_high={entry['T_high']:.1f}  T_low={entry['T_low']:.1f}  "
                      f"→ GLOBAL={entry['rb_global']}  LOCAL={entry['rb_local']}  pass={entry['rb_pass']}")
    ...
    fig.suptitle(
        "HETA + R-B 三分法：alpha 敏感度分析\n"
        "Star Hybrid Network (3,4,3)  |  "
        "T_high = Mean(B) + alpha×Std(B)   T_low = Median(B)",
        fontsize=12
    )
